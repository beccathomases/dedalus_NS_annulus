...
# -------------------------
# Restart / run labeling
# -------------------------
# Run from Re = 2
Re_target = 40.0
run_label = "Re40_40per_dt01"   # change this each run
restart_mode = "u_only"         # choices: "none", "u_only", "full"
restart_file = "Re40_20per_dt01/checkpoints/checkpoints_s1.h5"  # e.g. "Re2_from_rest/checkpoints/checkpoints_s1.h5"
restart_index = -1            # -1 means last saved write

#Re_target = 9
#run_label = "Re9_from_Re8"
#restart_file = "Re8_from_Re7p5/snapshots/snapshots_s1.h5"
#restart_mode = "u_only"
#restart_index = -1
...
# -------------------------
log_iter = 500         # print iter, t, dt every 200 steps
nan_check_iter = 100   # check for NaN/Inf every 100 steps
bc_check_iter = 10     # one early BC sanity check

# -------------------------
# Non-finite / NaN check controls
# -------------------------
nan_check_iter = 100   # check every 100 iterations
...
def ramp(t, t_ramp=1):
    #return 1 - np.exp(-t/t_ramp). turn off ramp for restart only, useful if starting from rest
    return 1
...
if restart_mode == "full":
    ...
    # Same-resolution velocity restart
    ...
    logger.info(f"Using BC/ramp time offset = {bc_time_offset:.6f}")

elif restart_mode == "u_refine":
    # Different-resolution velocity restart via interpolation in (phi,r)
    sim_time_old, uphi_old, ur_old, phi_old, r_old = read_u_task_from_h5(restart_file, index=restart_index)

    # Current grid at scale 1
    ...
    logger.info(f"Old sim time = {bc_time_offset:.6f}")
...
checkpoints = solver.evaluator.add_file_handler(
    f'{run_label}/checkpoints',
    sim_dt=stop_sim_time,
    max_writes=1,
    mode=file_handler_mode
)
...
# -------------------------
CFL = d3.CFL(solver, initial_dt=initial_dt, cadence=10, safety=0.8,
